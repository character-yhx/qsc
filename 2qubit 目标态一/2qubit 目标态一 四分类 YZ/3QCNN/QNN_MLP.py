...
for run in range(run_num):
    MLP_QNN = open(f'2qubit_target1_four_yz_QNN_MLP_episode{total_episode}_run{run+1}_训练loss_训练acc_测试loss_测试acc_时间.csv', 'w')
    ...
    for episode in range(total_episode):
        ...
        if episode % 1 == 0:
            ...
            print(f'2qubit_target1_four_yz_QNN_MLP, 回合 %d, 训练 loss %f, 训练 acc %f, 测试 loss %f, 测试 acc %f, 时间 %fs' % (episode, train_loss.data, correct_train / len(train_set_lable), test_loss.data, correct_test / len(test_set_lable), cost_time))
    ...
    if not os.path.exists(model_path):
        os.mkdir(model_path)
    torch.save(net.state_dict(), model_path + f'2qubit_target1_four_yz_QNN_MLP_episode{total_episode}_run{run+1}_model.pth')
    net.load_state_dict(torch.load(model_path + f'2qubit_target1_four_yz_QNN_MLP_episode{total_episode}_run{run+1}_model.pth'))
...
np.savetxt(f'2qubit_target1_four_yz_QNN_MLP_episode{total_episode}_{run_num}runs_时间.csv', time_runs, delimiter=',', fmt='%1.8f')
np.savetxt(f'2qubit_target1_four_yz_QNN_MLP_episode{total_episode}_{run_num}runs_训练loss.csv', train_loss_runs, delimiter=',', fmt='%1.8f')
np.savetxt(f'2qubit_target1_four_yz_QNN_MLP_episode{total_episode}_{run_num}runs_训练acc.csv', train_acc_runs, delimiter=',', fmt='%1.8f')
np.savetxt(f'2qubit_target1_four_yz_QNN_MLP_episode{total_episode}_{run_num}runs_测试loss.csv', test_loss_runs, delimiter=',', fmt='%1.8f')
np.savetxt(f'2qubit_target1_four_yz_QNN_MLP_episode{total_episode}_{run_num}runs_测试acc.csv', test_acc_runs, delimiter=',', fmt='%1.8f')
